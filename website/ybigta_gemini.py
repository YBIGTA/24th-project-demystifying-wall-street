...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = 'website/ybigta-414314-a438fffa0b91.json'
...
def get_economic_term_definitions(news_article: str, summary: str, economic_terms: List[str], count: int) -> (List[str], int):
    """
    ...
    if len(definitions) != len(economic_terms):
        ...
        definitions, count = get_economic_term_definitions(
            news_article, summary, economic_terms, count - 1)
...
def start(news_article: str) -> Dict[str, List[str]]:
    ...
    definitions, _ = get_economic_term_definitions(news_article, summary, terms, 3)
